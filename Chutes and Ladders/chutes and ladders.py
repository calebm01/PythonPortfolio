...
#Class that establishes the player, what attributes define the player, how the player will determine how many spaces they move,
#setting up the player to move, and also what conditions need to be met for the player to win the game
class Player(object):
...
#Class that creates the board and also sets up which spaces will be chutes and which spaces will be laders
#through the create_space function
class Board(object):
    def __init__(self):
        self.board = []
    def create_space(self):
        ...
        #Function to show the board to whoever's playing cleanly   
...
#Class centered around moving the player through the board
class Space(object):
    #__init__
    def __init__(self,move):
        self.move = move
    #Function to actually move the player pieces
    def move_player(self,player,board):
        oldpos = player.position
        player.position = player.position + self.move
        if self.move>0:
            print("this space has a ladder "+player.piece+"moves up",self.move,"spaces")
...
#Function used to ask the player for the amount of players that will be playing the board game
def ask_num(question,low,high):
    while True:
        ...
        if num_players in range(2,4+1):
            return num_players
        else:
            print("number not in correct range (2,4)")
        
     #Function to switch turns between the number of players   
...
def winner_grats():
    ...
    #Main Function that puts everything together in an executable format
def main():
